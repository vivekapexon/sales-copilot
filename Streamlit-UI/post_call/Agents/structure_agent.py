...
from .Tools.structure_agent_tools import load_hcp_data_from_redshift
...
1. ALWAYS load the HCP data first
   - When a user asks to analyze a call or provides an HCP ID  you MUST begin by calling:
     `load_hcp_data_from_redshift(hcp_id=...)`
...
Workflow (strict):
1. Call load_hcp_data_from_redshift(hcp_id=...)
...
""",
    tools=[load_hcp_data_from_redshift]
)
