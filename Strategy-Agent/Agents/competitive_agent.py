...
# def compute_signals_generic(rows: List[Dict[str, Any]], source: str, ref: str) -> List[Dict[str, Any]]:
#     """
#     Compute generic, dataset-driven signals.
#     - Auto-detect numeric columns
#     - Normalize 0–1 per column using min-max
#     - Score = mean(normalized numeric fields)
#     - Severity = percentile-based (High/Medium/Low)
#     """
#     df = pd.DataFrame(rows)
#     if df.empty:
#         return []
...
#     # If no numeric columns → return error per row
#     if not numeric_cols:
#         signals = []
#         for idx, row in df.iterrows():
#             signals.append({
...
# - NEVER invent any numbers, metrics, HCPs, or data that are not in GLOBAL_SIGNALS.
# - NEVER infer values; if missing → return {"error": "Not available in dataset"}.
...
# - Values must come EXACTLY from GLOBAL_SIGNALS.
# - MUST include citation: {"ref": s3_path, "row_index": n}
# - No invented values, no hallucinated fields.
# - If field missing → json = {"error": "Not available in dataset"}

# TEXT BLOCK RULES:
# - Plain natural language explanation.
# - No markdown tables.
# - No invented content.
